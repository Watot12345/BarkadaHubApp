...
async function updateMemberCount(club, card) {
    ...
    try {
        console.log('Fetching members for club_id:', club.id);
        ...
        console.log(`Member count from Supabase for club ${club.id}:`, count);
...
function renderClubCard(club) {
    console.log('Rendering club object:', club);
...
async function loadClubs() {
    ...
    try {
        ...
        if (userJoinedClub) return window.location.href = `joinedClub.html?clubId=${userJoinedClub.club_id}`;
        ...
        grid.innerHTML = `<p class="text-red-500">Failed to load clubs: ${err.message}</p>`;
